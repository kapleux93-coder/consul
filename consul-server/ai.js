...
function systemPrompt(w, chunks) {
  ...
  // Замер на стенде: на «а в рассрочку можно?» бот ответил «рассрочки у нас
  // нет» — верно и по базе — и всё равно позвал менеджера. Отрицательный ответ
  // он принял за плохую новость, которую надо эскалировать. Добавить человеку
  // там нечего, а разговор уже отдан.
  lines.push('- ответ отрицательный, но он есть в ЗНАНИЯХ: «рассрочки нет», «в воскресенье не работаем»,');
  lines.push('  «этого размера не бывает». Это полный ответ, а не плохая новость. Скажи прямо и предложи');
  lines.push('  то, что есть. Звать человека, чтобы он повторил твоё «нет», незачем.');
