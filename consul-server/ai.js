...
function systemPrompt(w, chunks) {
  ...
  // Ослабив правила передачи, мы едва не выменяли одну беду на худшую: вместо
  // «зову менеджера» бот начал сочинять правдоподобное. Замер на живой модели:
  // на вопрос об оплате, которой нет в базе, он выдал «наличными, картой или
  // переводом». Переданный разговор стоит денег, выдуманный факт — репутации.
  lines.push('- ВАЖНО: «не звать человека» НЕ значит «придумать ответ». Перечислить правдоподобные');
  lines.push('  варианты — способы оплаты, сроки, условия — которых нет в ЗНАНИЯХ, нельзя ни при каких условиях.');
  lines.push('  Нет факта — так и скажи: «про оплату уточню» — и передай диалог человеку (handoff = true).');
  lines.push('  Лучше признаться, что не знаешь, чем угадать: угаданную цену клиент запомнит как обещание.');
  ...
  lines.push('- Клиент спрашивает о том, чего в ЗНАНИЯХ нет, и придумать ответ ты не имеешь права.');
  lines.push('');
...
/* ============================================================ пробелы в базе
 *
 * Владелец загрузил прайс и считает, что готов. Клиент спрашивает про оплату
 * картой — бот зовёт менеджера. Таких дыр обычно пять-шесть, и все они
 * известны заранее: про них спрашивают в любом подобном бизнесе.
 *
 * Поэтому спрашиваем у модели не «что написать», а «чего здесь не хватает», и
 * задаём эти вопросы владельцу, пока он ещё в приложении. Ответ на вопрос,
 * заданный вовремя, дешевле переданного менеджеру разговора.
 * ========================================================================== */

/** Сколько дыр показываем за раз: длинный список закрывают не глядя. */
const GAPS_MAX = 6;

/**
 * Ищет вопросы, которые клиенты зададут, а ответа в материалах нет.
 * @returns {Promise<{questions:Array<{q:string, why:string}>, usage:object}>}
 */
async function gaps(w) {
  const ready = (w.knowledge || []).filter(k => k.ready && k.body);
  if (!ready.length) throw new Error('материалов пока нет — добавьте хотя бы один');

  // Берём начало каждого материала: для поиска дыр важна тема, а не весь текст.
  const digest = ready.slice(0, 12)
    .map(k => '### ' + k.title + '\n' + String(k.body).slice(0, 1800))
    .join('\n\n');

  const sys = [
    `Ты проверяешь, готов ли бот-продавец компании «${w.biz.name || w.bot.name || 'компания'}» отвечать клиентам.`,
    w.biz.about ? 'Чем занимается компания: ' + w.biz.about : '',
    '',
    'Тебе дают материалы, которые загрузил владелец. Твоя работа — найти дыры:',
    'вопросы, которые клиенты этого бизнеса задают постоянно, а ответа в материалах нет.',
    '',
    'ПРАВИЛА',
    '- Спрашивай только то, чего в материалах действительно НЕТ. Есть ответ — это не дыра.',
    '- Только то, что спросит клиент. Не спрашивай про внутреннюю кухню, маркетинг и планы.',
    '- Вопрос задавай владельцу, а не клиенту: «Как у вас можно оплатить?», а не «Как я могу оплатить?».',
    '- Формулируй коротко и конкретно, чтобы на него можно было ответить одной строкой.',
    '- Сначала то, что спрашивают чаще: оплата, доставка, сроки, гарантия, возврат, режим работы, самовывоз.',
    `- Не больше ${GAPS_MAX} вопросов. Если материалы полные — верни пустой список, это нормальный ответ.`,
    '',
    'Ответ — один JSON:',
    '{"questions":[{"q":"вопрос владельцу","why":"какой вопрос клиента останется без ответа"}]}',
  ].filter(Boolean).join('\n');

  const out = await groq.chat({
    system: sys,
    messages: [{ role: 'user', content: 'Материалы компании:\n' + digest }],
    json: true, maxTokens: 700, temperature: 0.3, reasoning: 'low',
  });
  const p = groq.extractJson(out.text);
  const list = (p && Array.isArray(p.questions)) ? p.questions : [];
  return {
    questions: list.slice(0, GAPS_MAX)
      .map(x => ({ q: clean(x && x.q, 160), why: clean(x && x.why, 160) }))
      .filter(x => x.q),
    usage: out.usage || {},
  };
}
...
module.exports = {
  ...
  split, sliceByLines, KINDS, SPLIT_MAX_CHARS, gaps, GAPS_MAX,
};
