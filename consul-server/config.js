...
const cfg = {
  ...
  limits: {
    /* Числа взяты не с потолка, а из суточной квоты Groq. Бесплатный тариф даёт
     * 200 000 токенов в сутки на весь аккаунт, один ответ клиенту стоит около
     * 2700 — то есть примерно 75 ответов в день на ВЕСЬ сервис, а не на кабинет.
     * Стояло 5000, и это была фантазия: сервис упирался в квоту Groq задолго до
     * собственного лимита и просто начинал сыпать отказами. Лучше честно
     * упереться в свой предел и сказать об этом, чем в чужой и молча сломаться.
     * На платном ключе поднимите обе переменные — там квота другая. */
    dailyPerWorkspace: num(process.env.LIMIT_DAILY_AI, 25),    // ответов в сутки на кабинет
    dailyGlobal: num(process.env.LIMIT_DAILY_GLOBAL, 70),      // ответов в сутки на весь сервис
