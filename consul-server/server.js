...
async function handlePlatformUpdate(update) {
  ...
  if (!m) {
    ...
    } else if (/^\/id\b/.test(msg.text)) {
      // Свой id нужен, чтобы вписать его в ADMIN_IDS и получать оповещения о
      // сбоях. Бот его и так видит в каждом сообщении — ходить за ним к чужим
      // сервисам незачем.
      const admin = cfg.adminIds.includes(Number(msg.userId));
      await tg.sendMessage(BOT_TOKEN, msg.chatId,
        'Ваш Telegram id: ' + msg.userId + '\n\n' +
        (admin
          ? 'Он уже в ADMIN_IDS — оповещения о сбоях приходят сюда.'
          : 'Впишите его в переменную ADMIN_IDS на хостинге, чтобы получать сюда ' +
            'оповещения о сбоях сервиса. Несколько id — через запятую.')).catch(() => {});
    }
    return;
  }
  const code = m[1];
...
async function boot() {
  ...
  /* Суточная квота Groq кончилась — это не «модель подтормаживает», а полное
     молчание бота до обновления квоты. Владелец сервиса должен узнать об этом
     от нас, а не от своих пользователей. */
  groq.onDailyLimit = () => alertAdmins('groq-daily',
    'Суточная квота токенов Groq исчерпана — бот перестал отвечать клиентам, ' +
    'все разговоры уходят менеджерам.\n\nНа бесплатном тарифе это около 75 ответов в сутки ' +
    'на весь сервис. Квота обновится сама; если это повторяется — пора переходить на платный ключ.');
